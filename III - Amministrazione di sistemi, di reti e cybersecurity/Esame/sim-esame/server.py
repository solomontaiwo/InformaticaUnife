...
while True:
    ...
    # eseguire il comando per vedere tutti i processi dell'utente
    # comando da eseugire 'ps -u <sutente>'
    cmd = 'ps -u ' + sutente
    prc = subprocess.run(cmd.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    output = prc.stdout

    # scrivere sulla socket il risultato
    # output da str a bytes
