	 ...
	 * seconda del sistema operativo */
        ...
        for(;;) {
                ...
                if ((pid = fork()) < 0) {
                        ...
                        for (;;) {
				uint8_t len[2];
				char nome[MAX_REQUEST_SIZE], annata[MAX_REQUEST_SIZE], template[256];
				size_t nome_len, annata_len;
				int fd, status, pid_n1, pid_n2, pipe_n1n2[2];
				...
				/* Aspetto terminazione nipot1 per sincerarmi
